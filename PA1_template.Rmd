...
```
**Mean of steps per day:** `r mean_steps_per_day`  
**Median of steps per day:** `r median_steps_per_day`
...
The 5-min **interval** with the maximun averaged steps is **`r mean_steps_per_interval[mean_steps_per_interval$steps==max(mean_steps_per_interval$steps),"interval"]`** with **`r mean_steps_per_interval[mean_steps_per_interval$steps==max(mean_steps_per_interval$steps),"steps"]`** steps
...
The total number of **rows with missing values** is `r nrow(activity[is.na(activity$steps) | is.na(activity$date) | is.na(activity$interval),])`
...
The **mean** of steps per day after the replacement of missing values is `r mean_steps_per_day_fill_na`  
The **median** of stepts per day after the replacement of missing values is `r median_steps_per_day_fill_na`
